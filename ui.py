...
from PyQt5.QtWidgets import (
    ...
    QWidget, QLineEdit, QPushButton, QTabWidget, QTreeView, QTableView, QHeaderView, QAbstractItemView
)
...
class ProcessViewer(QMainWindow):
    def __init__(self):
        ...
        self.layout = QHBoxLayout(self.container)  # Change to horizontal layout

        # Tabs
        self.tabs = QTabWidget()
        self.tabs.setTabPosition(QTabWidget.West)  # Move tabs to the left
        ...
        self.previous_update_time = time.time()  # To calculate time intervals
    ...
    def init_process_tab(self):
        ...
        self.list_model.setHorizontalHeaderLabels(
            ["Name", "PID", "User", "Priority", "CPU %", "Memory %", "Disk Read (KB)", "Disk Write (KB)", "GPU %", "Power %"]
        )
    ...
    def update_list_data(self):
        ...
        # Fetch all processes using Proc
        all_procs = self.get_all_procs()

        # Apply filtering if a regex is set
        if self.filter_regex:
            try:
                filtered_procs = filter_process_by_name(all_procs, self.filter_regex)
            except re.error:
                filtered_procs = []  # Invalid regex will show no results
        else:
            filtered_procs = all_procs

        # Populate the list model with the filtered results
        for proc in filtered_procs:
            try:
                ...
                data = [
                    ...
                    0.0,  # Placeholder for GPU usage
                    0.0,  # Placeholder for power usage
                ]
                self.list_model.appendRow([QStandardItem(str(item)) for item in data])
            except Exception:
                continue  # Skip processes with errors
    ...
    def update_tree_data(self):
        """Update the process tree model while maintaining scroll position."""
        ...
        # Clear the tree model without resetting the view
    ...
    def get_open_nodes(self):
        """Get the list of currently open nodes in the TreeView."""
        open_nodes = set()
    ...
    def restore_open_nodes(self, open_nodes):
        """Restore the open state of nodes in the TreeView."""
    ...
    def on_search_click(self):
        """Filter processes based on search entry."""
