...
def snapshot_info(proc_class, resource_class, filename='snapshot_log.txt'):
    ...
    # 3. 소켓 정보 수집
    socket_ = socket_info.get_socket_info()
    ...
    with open(filename, 'a') as log_file:
        ...
        # 소켓 정보 기록
        log_file.write("Socket Information (TCP):\n")
        log_file.write(f"{'Local Address':<25} {'Remote Address':<25} {'PID':<6} {'Status'}\n")
        for laddr, raddr, pid, status in socket_[0]:  # TCP 정보
            log_file.write(f"{laddr or '-':<25} {raddr or '-':<25} {pid or '-':<6} {status or '-'}\n")

        log_file.write("\nSocket Information (UDP):\n")
        log_file.write(f"{'Local Address':<25} {'Remote Address':<25} {'PID':<6}\n")
        for laddr, raddr, pid in socket_[1]:  # UDP 정보
            log_file.write(f"{laddr or '-':<25} {raddr or '-':<25} {pid or '-':<6}\n")
...
if __name__ == "__main__":
    ...
    proc_instance = process.Proc(200)
